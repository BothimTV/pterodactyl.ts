...
async function startContainers() {
    ...
    console.log("Mysql is running: " + mysql.getIpAddress(network.getName()) + ":3006")
    console.log("Mysql is running: localhost:", mysql.getMappedPort(3306))

    let panelUrl = "http://" + mysql.getIpAddress(network.getName()).split(".").map((v, i) => i === 3 ? 4 : v).join(".")
...
async function executeMysql(port: number) {
    ...
    const queries = readFileSync("./tests/devSetup.sql", { encoding: "utf-8" }).split("\n")
...
export async function startWings(cfg: RawNodeConfiguration): Promise<StartedTestContainer> {
