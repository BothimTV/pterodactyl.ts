...
export class Allocation implements AllocationAttributes {
    ...
    public async setNotes(notes: string): Promise<void> {
        const endpoint = new URL(client.panel + "/api/client/servers/" + this.parentServer.identifier + "/network/allocations/" + this.id);
        const data = { notes: notes }
        const res = await client.api({ url: endpoint.href, method: "POST", data: data }) as RawAllocation
    ...
    public async setPrimary(): Promise<void> {
        const endpoint = new URL(client.panel + "/api/client/servers/" + this.parentServer.identifier + "/network/allocations/" + this.id + "/primary");
        const res = await client.api({ url: endpoint.href, method: "POST" }) as RawAllocation
    ...
    public async unassign(): Promise<void> {
        if (this.is_default) console.warn("This allocation is locally set as primary location, this will most likely be rejected by the panel!")
        const endpoint = new URL(client.panel + "/api/client/servers/" + this.parentServer.identifier + "/network/allocations/" + this.id);
        await client.api({ url: endpoint.href, method: "DELETE" }, [{code: 400, message: "Cannot delete the primary allocation for a server."}])
    }
