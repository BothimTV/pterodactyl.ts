...
export class Backup implements BackupAttributes {
    ...
    public async downloadUrl(): Promise<URL> {
        const endpoint = new URL(client.panel + "/api/client/servers/" + this.parentServer.identifier + "/backups/" + this.uuid + "/download");
    ...
    public async restore(deleteFiles: boolean = false): Promise<void> {
        const endpoint = new URL(client.panel + "/api/client/servers/" + this.parentServer.identifier + "/backups/" + this.uuid + "/restore");
        await client.api({ url: endpoint.href, method: "POST", data: { truncate: deleteFiles } })
    }

    private async lockRequest() {
        const endpoint = new URL(client.panel + "/api/client/servers/" + this.parentServer.identifier + "/backups/" + this.uuid + "/lock");
        const res = await client.api({ url: endpoint.href, method: "POST" }) as RawBackup
