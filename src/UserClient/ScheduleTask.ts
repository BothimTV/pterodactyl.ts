...
export class ScheduleTask implements ScheduleTaskAttributes {
    ...
    private async updateThis(data: any) {
        const endpoint = new URL(client.panel + "/api/client/servers/" + this.parentSchedule.parentServer.identifier + "/schedules/" + this.id);
        const res = await client.api({ url: endpoint.href, method: "POST", data: data }) as RawScheduleTask
